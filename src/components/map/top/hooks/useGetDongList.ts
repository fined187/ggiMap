...
const fetchDongList = async (si: string, gu: string) => {
  try {
    ...
    if (data.success) {
      const addArray = data.data.umds.length % 3 === 0 ? null : Array(3 - (data.data.umds.length % 3)).fill({
        umd: ' ',
        x: 0,
        y: 0,
      })
...
export default function useGetDongList() {
  ...
  return useQuery<DongProps[], Error>('dongList', () => fetchDongList(juso.bottomSido, juso.bottomGungu))
}
